"use client";
import { getBidsForAuctions } from "@/app/Services/auctionService";
...
function BidList({ user, auction }: Props) {
    ...
    const highbid = bids.reduce(
        (previous, current) =>
            previous > current.amount ? previous : current.bidStatus.includes("Accepted") ? current.amount : previous,
        0
    );
